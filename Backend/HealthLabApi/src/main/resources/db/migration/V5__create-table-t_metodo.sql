...
('COLORIMÉTRICO', 'O método colorimétrico é uma técnica de análise química que se baseia na medição da absorção ou intensidade da cor de uma solução química. É amplamente utilizado para determinar a concentração de uma substância em uma solução, identificar a presença de substâncias específicas e medir as propriedades químicas de uma amostra com base na cor que ela apresenta quando reage com um reagente específico.
